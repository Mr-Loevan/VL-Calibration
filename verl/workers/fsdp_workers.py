...
class FSDPWorker(Worker):
    ...
    def compute_vision_kl(self, data: DataProto):
        
        from verl.utils.extract_vision_kl import batch_compute_vision_kl
        import numpy as np
        ...
        if old_log_probs is None or aug_log_probs is None or response_ids is None:
            print("[DEBUG] Missing required tensors for vision JS computation")
            return DataProto(non_tensor_batch={})
        
        if not hasattr(self, 'tokenizer') or self.tokenizer is None:
            print("[DEBUG] No tokenizer available for vision JS computation")
            return DataProto(non_tensor_batch={})
        
        old_log_probs = old_log_probs.cpu() if hasattr(old_log_probs, 'cpu') else old_log_probs
        aug_log_probs = aug_log_probs.cpu() if hasattr(aug_log_probs, 'cpu') else aug_log_probs
        response_ids = response_ids.cpu() if hasattr(response_ids, 'cpu') else response_ids
        response_masks = response_masks.cpu() if hasattr(response_masks, 'cpu') else response_masks
        
        vision_kl_results = batch_compute_vision_kl(
            old_log_probs,
            aug_log_probs,
            response_ids,
            self.tokenizer,
        ...
        vision_kl_values = [result[0] for result in vision_kl_results]
        vision_kl_token_counts = [result[1] for result in vision_kl_results]
        
        valid_kl_count = sum(1 for kl in vision_kl_values if kl is not None)
        if self.rank == 0:
            print(f"[DEBUG] Computed vision reverse KL: {valid_kl_count}/{len(vision_kl_values)} valid samples")
            print(f"[TRACE] compute_vision_kl called with batch size: {len(vision_kl_values)}")
        
        return DataProto(
            non_tensor_batch={
                "vision_kl": np.array(vision_kl_values, dtype=object),
                "vision_kl_token_count": np.array(vision_kl_token_counts, dtype=object)
            }
        )
    ...
    def _process_aug_multi_modal_inputs(self, data: DataProto):
        ...
        for multi_modal_data in data.non_tensor_batch["multi_modal_data"]:
            ...
            if "images" in multi_modal_data:
                for image in multi_modal_data["images"]:
                    processed_image = process_image(image, min_pixels, max_pixels)
                    augmented_image = perc_utils.augment_image(processed_image)
        ...
        with self.ulysses_sharding_manager:
            ...
            if isinstance(result, tuple):
                ...
                if len(result) > 1 and result[1] is not None:
                    import numpy as np
                    vision_entropy = result[1]
                    if isinstance(vision_entropy, list):
                        vision_entropy = np.array(vision_entropy, dtype=object)
                    non_tensors["vision_entropy"] = vision_entropy
                    print(f"[DEBUG] Received vision_entropy from compute_log_prob: shape={vision_entropy.shape if hasattr(vision_entropy, 'shape') else len(vision_entropy)}")
                if len(result) > 2 and result[2] is not None:
                    vision_token_count = result[2]
                    if isinstance(vision_token_count, list):
                        vision_token_count = np.array(vision_token_count, dtype=object)
                    non_tensors["vision_token_count"] = vision_token_count
                    print(f"[DEBUG] Received vision_token_count from compute_log_prob")
            else:
                log_probs = result
                print(f"[DEBUG] compute_log_prob returned single value (no vision data)")
    ...
    def compute_ref_log_probs(self, data: DataProto):
        ...
        with self.ulysses_sharding_manager:
            data = self.ulysses_sharding_manager.preprocess_data(data)
            result = self.ref_policy.compute_log_prob(data=data)
